variable "vpc_availability_zone_Hong_Kong" {
  type        = list(string)
...
resource "aws_subnet" "public_subnet_Hong_Kong" {
  ...
  count             = length(var.vpc_availability_zone_Hong_Kong)
  cidr_block        = cidrsubnet(aws_vpc.hong_kong.cidr_block, 8, count.index + 1)
  availability_zone = element(var.vpc_availability_zone_Hong_Kong, count.index)
  provider          = aws.hong_kong
  tags = {
...
resource "aws_ec2_transit_gateway_vpc_attachment" "hong_kong-TGW-attachment" {
  subnet_ids         = aws_subnet.public_subnet_Hong_Kong[*].id
